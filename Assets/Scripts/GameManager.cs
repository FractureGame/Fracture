    ...
    public class GameManager : MonoBehaviourPunCallbacks
    {
        ...
        public void Start()
        {
            ...
            else if (SceneManager.GetActiveScene().name == "BossRoom")
            {
                if (PhotonNetwork.IsMasterClient)
                {
                    PhotonNetwork.Instantiate(playerTopPrefab.name, new Vector3(100, 1f,0f), Quaternion.identity, 0);
                }
                else
                {
                    PhotonNetwork.Instantiate(playerBotPrefab.name, new Vector3(100f, 1f,0f), Quaternion.identity, 0);
                }
            }
        }
